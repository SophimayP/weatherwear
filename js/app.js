      ...
      if (status == google.maps.GeocoderStatus.OK) {
        for (var location of locations) {
          if ($.inArray('locality', location.types) != -1) {
            ...
            var handlebarsTempbit = '<div>{{#each 'city'}}<div class="anEntry"><div class="entryImg">{{scale}}</div><p>Took a raincoat: {{rain}}</p></div>{{/each}}</div>'; 
            document.getElementById("putCityandTemp").innerHTML = handlebarsTempbit;  
            /* -------------- WEATHER ------------ */
